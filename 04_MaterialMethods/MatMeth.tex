...
%    \captionsetup{format=hang, labelfont=bf, textfont=it}
%    \caption{The geological model of the Chtouka aquifer, as defined by \cite{Horn.2021}. The $xy$-plane is oriented along the four cardinal directions with the positive $x$-direction pointing to the north (arrow). In the $xy$-plane cell edges have a length of $500 \, \textrm{m}$. For presentation purposes, a $z$-magnification with a factor of 10 is applied.}
%    \label{Fig-GeolMod}
%\end{figure}
...
% correction option 1: This model deals with an unconfined aquifer. Therefore, \textcite{Horn.2021} conducted a literature research on values for the specific yield $S_y$ for the different materials. Values for the specific yield can either be measured directly or approximated as equalling the effective porosity. The identified range of values for these two parameters are shown in Table \ref{Tab-MatPropsRange}. In MODFLOW cells at the top of the grid can fall dry. If this happens, the affected cells are being deactivated by the model for the rest of the simulation. To avoid convergence problems due to this issue, here the aquifer is modelled using the "confined" option by MODFLOW. Thus, the identified values for the specific yield have to be converted to values of the specific storage $S_s$ as described in Section \ref{Sec-TheoFundTerms}/Equation \eqref{Eq-SsSy} using the aquifer thickness $b$. However, here the aquifer thicknesses vary over the model.
...
% until here the revised paragraph on S_y, S_s etc.
...
In the previous study by \textcite{Horn.2021}, the error of the steady state model was estimated through these two measures. 
